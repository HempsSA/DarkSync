...
class Job:
    ...
    workers: int = max(2, min(16, os.cpu_count() or 4))
    tolerance: int = 2
    include: str = "*"
...
def add_history(
    ...
    HISTORY_FILE.write_text(json.dumps(rows[-5000:], indent=2), encoding="utf-8")
...
def copy_atomic(src: Path, dst: Path, job: Job, cancel: threading.Event):
    ...
    try:
        ...
        if job.verify and digest(src, cancel) != digest(tmp, cancel):
            raise IOError("Post-copy verification failed")
...
def send_email_notification(n: NotifyConfig, subject: str, body: str, success: bool):
    if not n.enabled:
        return
...
def send_ntfy_notification(n: NotifyConfig, subject: str, body: str, success: bool):
...
def send_notification(job: Job, subject: str, body: str, success: bool):
    n = job.notify
